...
function Alertas() {
  ...
  const obtenerCompradores = useCallback(async () => {
    try {
      ...
      const ids = new Set();
      data.forEach((precio) => {
        if (precio.comprador && !ids.has(precio.comprador._id)) {
          ids.add(precio.comprador._id);
          unicos.push(precio.comprador);
        }
      });
      setCompradores(unicos);
    } catch (error) {
      console.error('Error al obtener compradores:', error.message);
    }
  }, [API_URL]);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-[#6B5A4D]">
              Te avisamos cuando el precio llegue al valor que definiste.
            </p>
          </div>
          <button
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[#8B7355] text-sm mt-0.5">
                        Avisar cuando el precio supere <span className="font-bold text-[#C8A96E]">${Number(alerta.precioMinimo).toLocaleString()}</span>
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 shrink-0">
                    <button
                      onClick={() => handleToggle(alerta._id)}
                    ...
                    <button
                      onClick={() => setModalEliminar(alerta._id)}
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleCrear} className="space-y-4">
              <div>
                ...
                >
                  ...
                  {compradores.map((comprador) => (
                    <option key={comprador._id} value={comprador._id}>{comprador.nombreempresa}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <label key={canal.key} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors ${form.canales[canal.key] ? 'border-[#C8A96E] bg-[#FFF8E7]' : 'border-gray-200 bg-gray-50'}`}>
                    ...
                    <input
                      type="checkbox"
                      checked={form.canales[canal.key]}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 bg-[#2C1A0E] text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-[#3D1F0F] transition-colors">
                  Crear alerta
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backdropFilter: 'blur(4px)', backgroundColor: 'rgba(0,0,0,0.3)' }}>
          <div className="bg-white rounded-2xl p-8 w-80 shadow-xl text-center">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleEliminar} className="flex-1 bg-[#2C1A0E] text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-[#3D1F0F] transition-colors">
                Eliminar
              </button>
            </div>
          </div>
