...
function Inicio() {
  ...
  // Scroll helper para no logueados
  const scrollTo = (ref) => {
    setTimeout(() => ref.current?.scrollIntoView({ behavior: 'smooth' }), 100);
  };

  // Scroll helper para logueados
  ...
  const pasos = [
    {
      ...
      detalle: 'Los precios se actualizan a lo largo del día. Puedes ver el precio por carga, por kilo y la variación respecto al día anterior.',
    },
    {
      paso: '02', icono: '🔔',
  ...
  const beneficios = [
    { icono: '⏱️', titulo: 'Ahorra tiempo', desc: 'Ya no tienes que ir físicamente a preguntar precios en cada compra. Con un par de clics sabes el panorama completo.' },
    { icono: '💰', titulo: 'Vende al mejor precio', desc: 'Al comparar varios compradores, puedes elegir al que mejor paga ese día. Esa diferencia puede ser significativa en una carga.' },
    { icono: '📱', titulo: 'Funciona desde el celular', desc: 'Diseñado para usarse desde el teléfono en el campo. No necesitas computador ni conocimientos técnicos.' },
    { icono: '🆓', titulo: 'Completamente gratis', desc: 'Para los caficultores no tiene ningún costo. La consulta de precios es libre, y la cuenta también es gratuita.' },
    { icono: '📍', titulo: 'Información local', desc: 'Los precios son de compradores reales en tu zona, no promedios nacionales. Lo que ves es lo que hay en tu municipio.' },
    { icono: '🔄', titulo: 'Siempre actualizado', desc: 'Los compradores actualizan sus precios durante el día. Tú siempre ves la información más reciente disponible.' },
  ];

  const faqs = [
    { pregunta: '¿Necesito crear una cuenta para ver los precios?', respuesta: 'No. Puedes consultar los precios del día sin registrarte. La cuenta gratuita desbloquea funciones adicionales como alertas de precio, historial y predicciones detalladas.' },
    { pregunta: '¿Con qué frecuencia se actualizan los precios?', respuesta: 'Los compradores pueden actualizar sus precios en cualquier momento del día. En general, los precios se renuevan cada mañana y pueden cambiar a lo largo del día según el mercado.' },
    { pregunta: '¿CoffePrice cobra algo a los caficultores?', respuesta: 'No. Para los caficultores el servicio es completamente gratuito, tanto la consulta de precios como el registro de cuenta.' },
    { pregunta: '¿En qué municipios está disponible?', respuesta: 'Actualmente tenemos cobertura en 12 municipios del Huila. Estamos trabajando para expandirnos a más zonas cafeteras del país.' },
    { pregunta: '¿Cómo sé que los precios son reales?', respuesta: 'Los precios son publicados directamente por los compradores registrados en la plataforma. Cada publicación tiene fecha y hora para que puedas verificar su vigencia.' },
    { pregunta: '¿Qué es la predicción de precios?', respuesta: 'Es una estimación basada en el comportamiento reciente del mercado local que te indica si el precio tiene tendencia a subir, bajar o mantenerse. Es una referencia, no una garantía.' },
  ];

  // ─────────────────────────────────────────────────────────────────────────────
  // HERO — compartido, CTAs cambian según sesión
  // ─────────────────────────────────────────────────────────────────────────────
  const Hero = () => (
        ...
        <div className="overflow-hidden rounded-[34px] bg-[linear-gradient(135deg,#3A281C_0%,#5A3E2C_60%,#7A573D_100%)] px-5 py-6 text-[#FBF5EC] shadow-[0_28px_70px_rgba(58,40,28,0.22)] md:px-8 md:py-8 xl:px-10">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                ...
                {usuario ? (
                  ...
                  <>
                    ...
                    <Link
                      to="/register"
                      className="inline-flex items-center justify-center rounded-2xl border border-[#E1BE86]/40 bg-[#E1BE86]/10 px-5 py-3 text-sm font-semibold text-[#E8D8C5] transition hover:bg-[#E1BE86]/20"
                    >
                      Crear cuenta gratis 
                    </Link>
                  </>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 rounded-[26px] bg-white/5 p-4 backdrop-blur-sm lg:grid-cols-3">
            <div className="rounded-[22px] bg-white/6 p-4">
              ...
              <p className="mt-1 text-sm text-[#E6D8C8]">Sin complicarte, en pocos segundos sabes cómo está el mercado.</p>
            </div>
            <div className="rounded-[22px] bg-white/6 p-4">
              ...
              <p className="mt-1 text-sm text-[#E6D8C8]">Compara antes de moverte y evita vender sin referencia.</p>
            </div>
            <div className="rounded-[22px] bg-white/6 p-4">
              ...
              <p className="mt-1 text-sm text-[#E6D8C8]">Mira si mañana podría subir, bajar o seguir parecido.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="px-4 py-6 md:px-8 md:py-8">
      <div className="mx-auto max-w-7xl">
        <div className="text-center">
          ...
          <h2 className="mt-3 text-2xl font-black text-[#2F241C] md:text-3xl">Voces del campo</h2>
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // VISTA LOGUEADO — diseño original
  // ─────────────────────────────────────────────────────────────────────────────
  if (usuario) {
    return (
      <div className="w-full bg-[linear-gradient(180deg,#F3E9DA_0%,#F7F2E8_42%,#EEE1CE_100%)]">
        ...
        {/* "Cómo funciona" colapsable — exactamente igual al original */}
        {mostrarComoFunciona && (
          <section ref={comoFuncionaRef} className="px-4 py-6 md:px-8 md:py-8">
            <div className="mx-auto max-w-7xl rounded-[30px] bg-[#F7EFE3] px-5 py-6 shadow-[0_12px_30px_rgba(110,86,60,0.08)] ring-1 ring-[#E7D7BF] md:px-8 md:py-8">
              ...
              <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                {[
                  { paso: '01', titulo: 'Mira tu zona', desc: 'Consulta los compradores activos del municipio donde estás.' },
                  { paso: '02', titulo: 'Compara precios', desc: 'Revisa quién está pagando mejor antes de moverte.' },
                  { paso: '03', titulo: 'Apóyate en la predicción', desc: 'Mira si mañana podría subir, bajar o seguir parecido.' },
                  { paso: '04', titulo: 'Entra para más funciones', desc: 'Al iniciar sesión puedes ver más detalles, alertas y otras ayudas.' },
                ].map((item, i) => (
                  <div key={i} className="rounded-3xl bg-white px-4 py-4 shadow-[0_10px_24px_rgba(96,73,47,0.07)] ring-1 ring-[#E7D7BF]">
                    <span className="inline-flex rounded-full bg-[#F3E5CE] px-3 py-1 text-[11px] font-bold tracking-[0.14em] text-[#7A5428]">
                      {item.paso}
                    </span>
                    <h3 className="mt-4 text-base font-black text-[#2F241C]">{item.titulo}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-[#6D5E53]">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // VISTA NO LOGUEADO — versión informativa completa
  // ─────────────────────────────────────────────────────────────────────────────
  return (
    <div className="w-full bg-[linear-gradient(180deg,#F3E9DA_0%,#F7F2E8_42%,#EEE1CE_100%)]">
      <Navbar />
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12 lg:items-center">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={i} className="rounded-2xl bg-[#FBF5EC] p-4 ring-1 ring-[#EDD9BF]">
        ...
        <div className="mx-auto max-w-7xl">
          <div className="rounded-[30px] bg-[#F7EFE3] px-5 py-8 shadow-[0_12px_30px_rgba(110,86,60,0.08)] ring-1 ring-[#E7D7BF] md:px-8 md:py-10">
            ...
            <div className="mt-10 space-y-4">
              {pasos.map((item, i) => (
                <div key={i} className="rounded-3xl bg-white px-5 py-5 shadow-[0_8px_20px_rgba(96,73,47,0.07)] ring-1 ring-[#E7D7BF]">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm leading-relaxed text-[#5E4B3A]">{item.desc}</p>
      ...
      <section ref={beneficiosRef} className="px-4 py-8 md:px-8 md:py-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {beneficios.map((b, i) => (
              <div key={i} className="rounded-[26px] bg-white p-5 shadow-[0_10px_24px_rgba(96,73,47,0.07)] ring-1 ring-[#E7D7BF]">
      ...
      {/* Sin cuenta vs. Con cuenta */}
      <section className="px-4 py-8 md:px-8 md:py-10">
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            <span className="inline-flex rounded-full bg-[#EDD9BF] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-[#7A5428]">
              ¿Porque deberia registrarme?
            </span>
            <h2 className="mt-3 text-2xl font-black text-[#2F241C] md:text-3xl">
      ...
      <section className="px-4 py-8 md:px-8 md:py-10">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-8 space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className="rounded-[20px] bg-white ring-1 ring-[#E7D7BF] overflow-hidden shadow-[0_6px_16px_rgba(96,73,47,0.06)]">
                <button
