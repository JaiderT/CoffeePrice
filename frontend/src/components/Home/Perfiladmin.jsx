                ...
                <div className="p-4 space-y-2">
                  {usuariosFiltrados.map((u, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-[#F5ECD7]/50 rounded-xl hover:bg-[#F5ECD7] transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`text-xs px-2 py-1 rounded-full font-semibold ${u.rol === 'admin' ? 'bg-purple-100 text-purple-700' :
                            u.rol === 'comprador' ? 'bg-blue-100 text-blue-700' :
                              'bg-green-100 text-green-700'}`}>
                          {u.rol}
                        </span>
                        <span className={`text-xs px-2 py-1 rounded-full font-semibold ${u.estado === 'activo' ? 'bg-green-100 text-green-700' :
