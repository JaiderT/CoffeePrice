...
export function AlertasProvider({ children }) {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        if ('Notification' in window && Notification.permission === 'granted') {
          const nuevasAlertas = data.filter(
            (alerta) => !alertasNotificadasRef.current.has(alerta._id)
          );
  ...
  const confirmarAlerta = async (alerta) => {
    ...
    const nuevas = alertasDisparadas.filter((item) => item._id !== alerta._id);
    setAlertasDisparadas(nuevas);
    if (nuevas.length === 0) setMostrarBanner(false);
    alertasNotificadasRef.current.delete(alerta._id);
  };
  ...
  useEffect(() => {
    alertasNotificadasRef.current = new Set();
  }, [usuario?.id]);
  ...
  return (
    <AlertasContext.Provider value={{ alertasDisparadas, mostrarBanner, cerrarBanner }}>
      ...
      {usuario?.id && mostrarBanner && alertasDisparadas.length > 0 && (
        <div className="fixed top-4 right-4 z-100 max-w-sm w-full space-y-2">
          {alertasDisparadas.map((alerta) => {
            ...
            return (
              <div key={alerta._id} className="bg-white rounded-2xl shadow-xl border border-[#C8A96E] p-4">
